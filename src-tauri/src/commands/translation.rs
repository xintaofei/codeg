...
/// itself (it is rebuilt from the provider list on save anyway). Shared by every
...
/// `provider_id` aims the probe at one list row; absent, the first active
...
/// The one endpoint's live state for the settings page's status strip. The
/// endpoint identity comes from the stored settings; the failure streak, the
/// cooldown, and the session disable are process-wide runtime memory.
pub async fn translation_pool_status_core(
    conn: &DatabaseConnection,
) -> crate::translation::endpoint::EndpointStatus {
    let settings = translation::settings::load(conn).await;
    translation::endpoint::status(&settings)
}
...
/// The manual "this endpoint is fixed" action: clear the runtime failure
/// streak, the automatic cooldown, and the session disable, so everything
/// starts counting fresh. Runtime memory only — no settings or db involved.
pub async fn translation_provider_reset_core() -> Result<(), AppCommandError> {
    translation::endpoint::reset_session();
    Ok(())
}

/// The manual "keep this endpoint out" action: session-disable the runtime
/// state (a restart or a reset clears it). The fresh status comes with the
/// reply so the caller's status strip updates without a second round trip.
pub async fn translation_provider_disable_core(
    conn: &DatabaseConnection,
) -> Result<crate::translation::endpoint::EndpointStatus, AppCommandError> {
    translation::endpoint::disable_session();
    let settings = translation::settings::load(conn).await;
    Ok(translation::endpoint::status(&settings))
}

/// Put the endpoint on a timed cooldown without ending its session: it sits
/// out for `seconds`, or the saved settings' default when no explicit length
/// is given. Returns the fresh status alongside the action.
pub async fn translation_provider_cooldown_core(
    ...
    translation::endpoint::cooldown_for(seconds);
    Ok(translation::endpoint::status(&settings))
}
...
/// `variant` is the caller's retry counter: bumping it re-requests a chunk
/// instead of serving the cached reply the caller is retrying away from.
...
#[cfg(feature = "tauri-runtime")]
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub async fn translation_provider_reset() -> Result<(), AppCommandError> {
    translation_provider_reset_core().await
}
...
pub async fn translation_provider_disable(
    ...
    translation_provider_disable_core(&db.conn).await
}
...
pub async fn translation_provider_cooldown(
    ...
    translation_provider_cooldown_core(&db.conn, seconds).await
}
