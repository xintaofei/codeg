...
describe("buildNumberedRequest / parseNumberedTranslation", () => {
  ...
  it("recovers segments squeezed onto one inline line (lenient second pass)", () => {
    // The observed failure: the model drops every line break and answers
    // "[1] 甲 [2] 乙" as one line — the strict line-anchored parse cannot
    // see it, the inline split can.
    expect(parseNumberedTranslation("[1] 甲 [2] 乙", 2)).toEqual(["甲", "乙"])
    expect(parseNumberedTranslation("[1] 甲\n[2] 乙\n[3] 丙", 3)).toEqual([
      "甲",
      "乙",
      "丙",
    ])
  ...
  it("refuses out-of-order inline numbering", () => {
    expect(parseNumberedTranslation("[2] 甲 [1] 乙", 2)).toBeNull()
  })

  it("refuses chatty preamble before the first inline marker", () => {
    expect(parseNumberedTranslation("说明：[1] 甲 [2] 乙", 2)).toBeNull()
  })

  it("keeps count=1 on the strict path only", () => {
    expect(parseNumberedTranslation("[1] 甲 [2] 乙", 1)).toEqual(["甲 [2] 乙"])
  })
})

describe("normalizeProtocolMarkerEcho", () => {
  it("restores paragraph breaks from an ascending marker echo", () => {
    // The observed shape: the whole list squeezed onto one line, the source's
    // "1. 2. 3." numbers rewritten as protocol markers. The space a segment
    // carries in front of the next marker stays; trim start only drops the
    // leading break the first marker left behind.
    expect(
      normalizeProtocolMarkerEcho(
        "[1] 苹果 [2] 香蕉 [3] 樱桃",
        "1. 苹果\n2. 香蕉\n3. 樱桃"
  ...
  it("restores a sequence that starts above one", () => {
    expect(normalizeProtocolMarkerEcho("前言 [3] 甲 [4] 乙", "甲\n乙")).toBe(
      "前言 \n\n甲 \n\n乙"
    )
  })

  it("leaves a lone marker untouched", () => {
    expect(normalizeProtocolMarkerEcho("[1] 甲", "甲")).toBe("[1] 甲")
  })

  it("leaves a non-ascending sequence untouched", () => {
    // "[2] 见上" style references never form a complete run.
    expect(normalizeProtocolMarkerEcho("[1] 甲 [3] 乙", "甲 乙")).toBe(
      "[1] 甲 [3] 乙"
    )
    expect(
      normalizeProtocolMarkerEcho("[1] 甲 [2] 乙 [2] 丙", "甲 乙 丙")
    ).toBe("[1] 甲 [2] 乙 [2] 丙")
  })

  it("leaves the reply untouched when the source itself carries marker shapes", () => {
    expect(
      normalizeProtocolMarkerEcho("[1] 甲 [2] 乙", "[1] 原文 [2] 原文")
    ).toBe("[1] 甲 [2] 乙")
  })

  it("needs exactly one space or tab after the bracket digits", () => {
    // "a[2]b" is not a marker match, so only one marker exists.
    expect(normalizeProtocolMarkerEcho("[1] 甲[2]乙[3]丙", "甲乙丙")).toBe(
      "[1] 甲[2]乙[3]丙"
    )
  })
})

describe("structureFlattenError", () => {
  it("rejects a reply that flattens a multi-line list", () => {
    const source =
      "1. 苹果\n2. 香蕉\n3. 樱桃\n4. 葡萄\n5. 柠檬\n6. 桃子\n7. 梨\n8. 西瓜"
    expect(structureFlattenError(source, "苹果、香蕉、樱桃等水果")).toBe(true)
  })

  it("accepts a reply to a hard-wrapped paragraph without line-end punctuation", () => {
    const source =
      "This is a long\nparagraph wrapped across\nseveral lines with\nno terminal punctuation"
    expect(structureFlattenError(source, "一段没有标点的长段落")).toBe(false)
  })

  it("stays silent below three structural lines", () => {
    expect(structureFlattenError("1. 苹果\n2. 香蕉", "苹果 香蕉")).toBe(false)
    expect(structureFlattenError("第一句。\n第二句。", "两句并成一行")).toBe(
      false
    )
  })
})
