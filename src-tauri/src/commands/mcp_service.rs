...
pub struct CodegMcpStatusSources<'a> {
    ...
    /// The experimental switch is managed in delegation settings, but can
    /// independently cause companion injection even with all regular groups off.
...
pub async fn codeg_mcp_service_status_core(
    ...
    let any_group_enabled = sources.continuation_enabled || tool_groups.iter().any(|g| g.enabled);
...
pub async fn get_codeg_mcp_service_status(
    ...
    #[cfg(feature = "tauri-runtime")]
    {
        Ok(codeg_mcp_service_status_core(CodegMcpStatusSources {
            ...
            continuation_enabled: coordinator.is_enabled().await,
        })
        .await)
    }
