...
interface SessionDetailsDialogProps {
   ...
   * session's turns. `undefined` (the sidebar card) makes the dialog derive the
...
export function SessionDetailsDialog({
  ...
  useEffect(() => {
    // Caller supplied stats (the detail panel reads them off the live runtime
    // session): render straight from props, nothing to fetch.
    if (!open || statsProp !== undefined) return
  ...
  const loadingStats = open && statsProp === undefined && result == null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="min-w-0 space-y-5 text-sm">
          ...
          {/* Identifiers, packed two-up to keep the dialog short. */}
