...
/// failed requests in a row (429s and hard errors alike) park the endpoint
...
/// One translation endpoint. PR1 dispatches to exactly one: the first
/// enabled, complete entry in [`TranslationSettings::providers`].
///
/// Rows written before the list existed stored a single endpoint in the flat
/// [`TranslationSettings`] fields; [`migrate_legacy`] synthesizes the list
/// from those on read, so every code path after `load` sees the list shape.
#[derive(Serialize, Deserialize, Clone, Debug, Default, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct ProviderConfig {
    /// Stable identity for runtime-state keying (the failure cooldown's
    /// memory) and the settings page's list rows. Empty on entries
    /// synthesized from legacy fields; `validate` fills one in on save.
    #[serde(default)]
    pub id: String,
    ...
    /// List membership. The global `enabled` is still the master switch; a
    /// disabled provider is skipped by endpoint selection without being
    /// deleted.
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Requests-per-minute ceiling. Stored for the settings page's shape;
    /// PR1 paces with fixed lane concurrency only, so the value is inert
    /// until the pacing layer lands.
    #[serde(default)]
...
impl ProviderConfig {
    ...
    /// Identifies the endpoint for runtime-state keying and the cache key:
    /// the same equivalence class the cache always partitioned on.
    /// Insensitive to base spelling (`https://host` ≡ `https://host/v1`),
    /// sensitive to dialect.
    ...
    /// entries are kept as settings-page drafts; endpoint selection skips
    /// them.
...
pub struct TranslationSettings {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// The configured endpoints, in user order. Legacy rows stored one
    /// endpoint in the flat fields below; [`migrate_legacy`] synthesizes a
    /// single-entry list from those on read, so this list is the source of
...
//
// This notifier covers the PERSISTED settings: a save anywhere (desktop
// command or web handler) must reach every open frontend, not just the window
// that saved. The app wires one callback per mode at startup that emits
// `translation-settings-changed`; listeners re-fetch the settings instead of
// the backend pushing a payload (which would leak the real keys if assembled
// carelessly).
...
impl TranslationSettings {
    ...
    /// The providers that may receive requests: entries the user has not
    ...
    /// "who is callable" once the feature is on.
    ...
    /// Consecutive failures before the endpoint auto-cools, as in force for
    ...
    /// The dialect of the first active member, for callers that need a
...
/// path after `load` sees the list shape regardless of what is on disk.
///
/// Runs on *read*, not on a stored-row rewrite: the flat fields stay the
/// source of truth for a row that has never been saved through the list-aware
...
fn migrate_legacy(mut settings: TranslationSettings) -> TranslationSettings {
    ...
    settings.providers.push(ProviderConfig {
        ...
        // keeps the runtime state keyed consistently).
        id: "legacy".to_string(),
        name: None,
...
pub fn validate(settings: TranslationSettings) -> Result<TranslationSettings, AppCommandError> {
    ...
    // Mirror the provider-list head into the legacy flat fields, so a row
    // carries both
    // shapes: old builds read the flat fields, list-aware paths read the
    // list, and neither sees an endpoint the other cannot. An empty list is
...
/// Validate one list member: trim, bound, normalize, keep its secret semantics
...
fn validate_provider(provider: ProviderConfig) -> Result<ProviderConfig, AppCommandError> {
    ...
    Ok(ProviderConfig {
        // A provider without an id gets one at validation time, so runtime-state
        // keying survives every later save (the id, not the list position, is
        // what the failure cooldown remembers).
...
/// flat legacy mirror is merged separately (it is the list head's shadow) and
...
pub async fn save(
    ...
    if merged.providers.is_empty() {
        ...
        // rebuilt from the provider-list head in `validate`.
...
mod tests {
    ...
    /// A row saved by the list-aware settings page: the endpoint lives in the
    ...
    fn host_and_v1_forms_share_one_provider_id() {
        ...
        let auto_detected = ProviderConfig {
            base_url: "https://api.anthropic.com".to_string(),
            ..Default::default()
        };
    ...
    fn provider_id_changes_with_the_format() {
        // The equivalence class lives on the member: it is what the cache key
        // and the runtime-state keying both partition on, so a dialect change
        // must split it — same host, different route = different endpoint.
    ...
    /// A legacy row (flat fields, no list) migrates to a one-member list on
    ...
    /// Saving a row with providers mirrors the head back into the flat
    /// fields, so an old build reading the same row still finds its endpoint.
    #[tokio::test]
    async fn saving_a_provider_mirrors_the_head_into_the_legacy_fields() {
        let db = fresh_in_memory_db().await;
        save(&db.conn, pooled()).await.expect("save the provider row");
        ...
        assert!(
            ...
            "validate assigns the stable id the runtime state keys on"
        );
    }
    ...
    fn enabling_requires_at_least_one_callable_provider() {
        let mut empty_list = pooled();
        empty_list.providers[0].enabled = false;
        assert!(validate(empty_list).is_err());
