...
use crate::translation::endpoint::EndpointStatus;
...
pub struct TestParams {
    ...
    /// Aims the test at one list row (the settings-page row being edited).
    #[serde(default)]
...
pub struct TranslateParams {
    ...
    /// The caller's retry counter: bumping it re-requests a chunk instead of
    /// serving the cached reply the caller is retrying away from.
    #[serde(default)]
    pub variant: u32,
...
pub struct ListModelsParams {
    pub settings: TranslationSettings,
    /// Aims the probe at one list row (the settings-page row being edited).
    #[serde(default)]
    pub provider_id: Option<String>,
}
...
pub async fn translation_provider_reset(
    Extension(state): Extension<Arc<AppState>>,
) -> Result<Json<EndpointStatus>, AppCommandError> {
    translation_provider_reset_core().await?;
    let settings = crate::translation::settings::load(&state.db.conn).await;
    Ok(Json(crate::translation::endpoint::status(&settings)))
}

pub async fn translation_provider_disable(
    Extension(state): Extension<Arc<AppState>>,
) -> Result<Json<EndpointStatus>, AppCommandError> {
    Ok(Json(translation_provider_disable_core(&state.db.conn).await?))
}
...
pub async fn translation_provider_cooldown(
    ...
    Ok(Json(
        translation_provider_cooldown_core(&state.db.conn, params.seconds).await?,
    ))
}
