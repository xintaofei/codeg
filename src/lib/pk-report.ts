...
import { getPkReportLabels } from "@/lib/pk-report-locales"
import {
...
export function buildPkReportHtml(
  ...
  const l = getPkReportLabels(locale)
  ...
  return `<!doctype html>
<html lang="${esc(locale)}"${locale.toLowerCase().startsWith("ar") ? ' dir="rtl"' : ""}><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<div class="content">${showcase}<section><div class="section-head"><h2>${l.ranking}</h2><span>${l.round} ${round.id}</span></div><div class="table-wrap"><table><thead><tr><th>#</th><th>${l.agent}</th><th>${l.score}</th><th>${l.time}</th><th>${l.tokens}</th><th>${l.turns}</th><th>${l.changes}</th><th>${l.files}</th></tr></thead><tbody>${rows}</tbody></table></div></section>${judge}<section class="details"><div class="section-head"><h2>${l.details}</h2></div>${details}</section><footer class="footer"><span>${esc(l.generatedBy)}</span><span>${new Date(round.createdAt).toLocaleString(locale)}</span></footer></div>
</main><script>
