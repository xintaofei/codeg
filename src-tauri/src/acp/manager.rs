    ...
    pub async fn send_prompt_linked_with_message_id(
        ...
        match self.send_prompt_inner(conn_id, blocks, user_message).await {
            Ok(()) => {
                ...
                if let (Some(cid), Some(title)) = (conversation_id_for_status, first_prompt_title) {
                    match conversation_service::seed_auto_title_if_empty(&db.conn, cid, title).await
                    {
                        Ok(true) => {
                            crate::commands::conversations::emit_conversation_upsert(
                                &emitter, &db.conn, cid,
...
mod tests {
    ...
    async fn fork_session_strips_existing_fork_prefix_without_stacking() {
        ...
        let result = mgr
            .fork_session(&db, "c-restack", None, None, None)
            .await
            .unwrap();
        let _ = join.await;
    ...
    async fn fork_session_strips_no_space_fork_prefix() {
        ...
        mgr.fork_session(&db, "c-nosp", None, None, None)
            .await
            .unwrap();
        let _ = join.await;
    ...
    async fn fork_session_sibling_inherits_the_manual_title_lock() {
        ...
        let result = mgr
            .fork_session(&db, "c-fork-lock", None, None, None)
            .await
            .unwrap();
        let _ = join.await;
