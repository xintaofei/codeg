...
pub async fn load_continuation_enabled(conn: &sea_orm::DatabaseConnection) -> bool {
    match app_metadata_service::get_value(conn, KEY_CONTINUATION_ENABLED).await {
        Ok(Some(raw)) => raw.trim().eq_ignore_ascii_case("true"),
        Ok(None) => false,
        Err(e) => {
            tracing::warn!("[continuation] reading the experiment flag failed ({e}); defaulting OFF");
            false
        }
    }
}
...
pub async fn set_continuation_settings(
    ...
    #[cfg(feature = "tauri-runtime")]
    {
        set_continuation_enabled_core(&db.conn, coordinator.inner(), settings.continuable_delegation_enabled)
            .await?;
        Ok(settings)
    }
    #[cfg(not(feature = "tauri-runtime"))]
