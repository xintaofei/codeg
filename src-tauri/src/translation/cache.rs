...
//! keyed by `sha256(masked_text:target_lang:provider_id:variant)` — content,
//! never an object reference — which is what keeps a `parts` array replacement
//! (stream → promoted turn → authoritative refetch) from invalidating a hit.
//! `variant` is the caller's retry counter: bumping it re-requests a chunk the
//! caller asked to redo instead of being handed the cached reply back.
...
/// `v5-variant`: the key gained the caller's retry `variant` as a component,
/// so every pre-variant entry reads under a different key space anyway; the
/// bump documents the shape change and keeps the lineage honest.
const KEY_VERSION: &str = "v5-variant";
...
impl TranslationCache {
    ...
    /// or model never serves output produced by the previous one, and
    /// `variant` so a caller retrying a chunk (bumping the variant) gets a
    /// fresh request instead of its own cached reply back.
    ///
    ...
    ///
    /// Each component is length-prefixed (`<len>:<bytes>`) so two tuples whose
    ...
    pub fn get(
        ...
        let key = Self::key_for(masked_text, target_lang, provider_id, variant);
...
mod tests {
    ...
    fn the_key_is_stable_for_the_same_input() {
        let a = TranslationCache::key_for("hello", "zh-CN", "p1", 0);
        let b = TranslationCache::key_for("hello", "zh-CN", "p1", 0);
        assert_eq!(a, b);
    }
    ...
    fn every_key_component_changes_the_key() {
        let base = TranslationCache::key_for("hello", "zh-CN", "p1", 0);
        assert_ne!(base, TranslationCache::key_for("hello!", "zh-CN", "p1", 0));
        assert_ne!(base, TranslationCache::key_for("hello", "ja", "p1", 0));
        assert_ne!(base, TranslationCache::key_for("hello", "zh-CN", "p2", 0));
    }

    /// A retry with a bumped variant must miss the variant-0 entry: the whole
    /// point of the dimension is that the caller asked for a different answer.
    #[test]
    fn a_bumped_variant_misses_the_cached_entry() {
        let (cache, _dir) = cache();
        cache.insert("hello", "zh-CN", "p1", 0, "\u{4f60}\u{597d}");
        assert_eq!(
            cache.get("hello", "zh-CN", "p1", 0).as_deref(),
            Some("\u{4f60}\u{597d}")
        );
        assert!(cache.get("hello", "zh-CN", "p1", 1).is_none());
        cache.insert("hello", "zh-CN", "p1", 1, "\u{60a8}\u{597d}");
        assert_eq!(
            cache.get("hello", "zh-CN", "p1", 1).as_deref(),
            Some("\u{60a8}\u{597d}")
        );
        // The variant-0 entry is untouched by the variant-1 write.
        assert_eq!(
            cache.get("hello", "zh-CN", "p1", 0).as_deref(),
            Some("\u{4f60}\u{597d}")
        );
    ...
    /// The separator must not let two different tuples collide by shifting
    /// the boundary between components.
    #[test]
    fn component_boundaries_do_not_collide() {
        assert_ne!(
            TranslationCache::key_for("a:b", "c", "d", 0),
            TranslationCache::key_for("a", "b:c", "d", 0)
        );
    }
    ...
    fn a_stored_translation_reads_back() {
        let (cache, _dir) = cache();
        cache.insert("hello", "zh-CN", "p1", 0, "你好");
        assert_eq!(cache.get("hello", "zh-CN", "p1", 0).as_deref(), Some("你好"));
    }

    #[test]
    fn a_miss_is_none() {
        let (cache, _dir) = cache();
        assert!(cache.get("nothing", "zh-CN", "p1", 0).is_none());
    }
    ...
    fn translations_survive_a_new_cache_over_the_same_dir() {
        ...
        first.insert("hello", "zh-CN", "p1", 0, "你好");

        let second = TranslationCache::new(dir.path().to_path_buf());
        assert_eq!(
            second.get("hello", "zh-CN", "p1", 0).as_deref(),
            Some("你好")
        );
    }
    ...
    fn re_inserting_a_key_overwrites_rather_than_duplicates() {
        let (cache, dir) = cache();
        cache.insert("hello", "zh-CN", "p1", 0, "你好");
        cache.insert("hello", "zh-CN", "p1", 0, "您好");

        assert_eq!(cache.get("hello", "zh-CN", "p1", 0).as_deref(), Some("您好"));
    ...
    fn languages_are_kept_in_separate_files() {
        let (cache, dir) = cache();
        cache.insert("hello", "zh-CN", "p1", 0, "你好");
        cache.insert("hello", "ja", "p1", 0, "こんにちは");

        assert!(dir.path().join("zh-CN.json").exists());
        assert!(dir.path().join("ja.json").exists());
        assert_eq!(cache.get("hello", "ja", "p1", 0).as_deref(), Some("こんにちは"));
    }
    ...
    fn the_disk_file_is_trimmed_to_the_byte_cap() {
        ...
        for i in 0..200 {
            cache.insert(&format!("src{i}"), "zh-CN", "p1", 0, &big);
        }
        ...
        assert_eq!(cache.get("src199", "zh-CN", "p1", 0).as_deref(), Some(&big[..]));
    }
    ...
    fn one_oversized_entry_is_still_written() {
        ...
        cache.insert("src", "zh-CN", "p1", 0, &huge);

        assert!(dir.path().join("zh-CN.json").exists());
        assert_eq!(cache.get("src", "zh-CN", "p1", 0).as_deref(), Some(&huge[..]));
    }
    ...
    fn a_corrupt_file_reads_as_empty_rather_than_failing() {
        ...
        assert!(cache.get("hello", "zh-CN", "p1", 0).is_none());
    ...
    fn stats_count_both_tiers() {
        let (cache, _dir) = cache();
        cache.insert("a", "zh-CN", "p1", 0, "A");
        cache.insert("b", "ja", "p1", 0, "B");
    ...
    fn clearing_empties_both_tiers() {
        ...
        assert!(cache.get("a", "zh-CN", "p1", 0).is_none());
