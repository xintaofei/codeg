...
pub struct CompanionContext {
    ...
    /// registered at injection time via `--custom-agents`. Empty when the
    /// parent has none (or predates the flag) — the schema is then served
    /// byte-identical to the embedded file.
