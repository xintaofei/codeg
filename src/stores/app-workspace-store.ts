  ...
  (set, get) => ({
    ...
    refreshConversations: async () => {
      ...
      try {
        ...
        // Defense in depth: the backend root listing already excludes delegation
        // children, but a child must never enter this store — every sidebar
        // section (folders / Chat / Pinned) and `stats` derive from it.
    ...
    // delegation children (parent_id / kind=delegate / delegation_call_id) are not
    // sidebar rows. New rows prepend (most-recent-first); existing rows replace in
    // place to keep their position.
