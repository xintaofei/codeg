...
type AppearanceContextValue = {
  ...
  /** 编辑器字体（Monaco 的 fontFamily，并驱动会话代码的 --font-code） */
 ...
 * 以及界面字体变量 --font-sans、会话代码变量 --font-code
 * （终端字体只走 xterm 选项）。
 *
...
export function AppearanceProvider({
  ...
  const setEditorFont = useCallback((id: string, custom = "") => {
    ...
    // Monaco 自己读 provider 状态。会话代码块读 --font-code，不改 --font-sans
    // 和界面上其它 font-mono。
    const stack = conversationCodeFontStack(id, custom)
    document.documentElement.style.setProperty("--font-code", stack)
  ...
  // mount 时重新解析并应用 --font-sans / --font-code，吸收跨版本字体目录变更
  // （inline 脚本写入的是旧版本已解析栈，可能与新目录不一致）。仅在确有漂移时才写
  // localStorage，避免每次加载都触发跨标签页 storage 事件。
  // 终端字体只走 xterm 选项，不在此落 CSS 变量。
  useEffect(() => {
    ...
    const code = conversationCodeFontStack(editorFont.id, editorFont.custom)
    document.documentElement.style.setProperty("--font-code", code)
    if (readStored(STORAGE_KEY_EDITOR_FONT_STACK) !== code) {
      persist(STORAGE_KEY_EDITOR_FONT_STACK, code)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  useEffect(() => {
    ...
    const rehydrateFonts = () => {
      ...
      // 界面字体落到 --font-sans；编辑器字体落到 --font-code（会话代码块）。
      // 终端字体由 xterm 读取 provider 状态后应用。
      document.documentElement.style.setProperty(
        "--font-sans",
      ...
      document.documentElement.style.setProperty(
        "--font-code",
        conversationCodeFontStack(ed.id, ed.custom)
      )
    }
